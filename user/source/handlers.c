	...
	if (TIM_GetITStatus(TIM2, TIM_IT_Update) != RESET) {
		...
		if (i < ui32RecBuffSize) {
			...
			if (i < (2 * ui32RecBuffSize)) { //
				ui8ADCConvertedArray_1[i - ui32RecBuffSize] =
						ADC_GetConversionValue(ADC2); // (uint8_t)(value);//, (uint8_t)value;// {ADC_GetConversionValue(ADC2)&0xff,;
				i++;
			}
			if (i == (2 * ui32RecBuffSize)) {
